    ...
    private void matchImages(String text) {
        ...
        while (imageMatcher.find()) {
            ...
            if (widthMatcher.find()) {
                int widthDip = parseStringToInteger(getTextBetweenQuotation(widthMatcher.group().trim().substring(6)));
                holder.setWidth(com.onfengma.androidprac.utils.ViewUtils.dipToPixels(widthDip, textView.getContext()));
            }

            heightMatcher = IMAGE_HEIGHT_PATTERN.matcher(image);
            if (heightMatcher.find()) {
                int widthDip = parseStringToInteger(getTextBetweenQuotation(heightMatcher.group().trim().substring(6)));
                holder.setHeight(com.onfengma.androidprac.utils.ViewUtils.dipToPixels(widthDip, textView.getContext()));
            }

 ...
 */
package com.onfengma.androidprac.views.listview;
...
public class XListView extends ListView implements OnScrollListener {
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
            if (onRefreshListener != null &&
                    mHeaderView.getVisibleHeight() > getHeaderActionHeight() && delta >= 0) {
                mHeaderView.setState(XListViewHeader.STATE_READY);
            } else if (delta < 0 && mHeaderView.getVisibleHeight() < getHeaderActionHeight()) {
                mHeaderView.setState(XListViewHeader.STATE_NORMAL);
            }
        }
    ...
    private void resetFooterHeight() {
        ...
        if (mLoadingMore && mFooterView.getFooterOffset() == 0) {
            scrollBackDistance = mFooterView.getVisibleHeight() - pullLoadMoreDelta;
        } else {
            scrollBackDistance = mFooterView.getVisibleHeight() - mFooterView.getFooterOffset();
        }

        if (scrollBackDistance > 0) {
            mScrollBack = SCROLLBACK_FOOTER;
            mScroller.startScroll(0, mFooterView.getVisibleHeight(), 0, -scrollBackDistance, SCROLL_DURATION);
            invalidate();
        }
    }
        ...
        switch (ev.getAction()) {
            ...
            default:
                ...
                if (getFirstVisiblePosition() == 0) {
                    // invoke refresh
                    if (mEnablePullRefresh && mHeaderView.getVisibleHeight() > getHeaderActionHeight()
                    ...
                    // invoke load more.
                    if (mEnableLoadMore && mFooterView.getVisibleHeight() - mFooterView.getFooterOffset() > pullLoadMoreDelta) {
                        startLoadMore();
                    }
                    resetFooterHeight();
                }

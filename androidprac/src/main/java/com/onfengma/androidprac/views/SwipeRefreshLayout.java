...
public class SwipeRefreshLayout extends RelativeLayout {
    ...
    private static final int REFRESH_TRIGGER_DISTANCE = 450;
    ...
    private boolean canRefreshUp = false;
    ...
    // Header & Footer
    private SwipeHeaderView swipeHeaderView;
...
//            }
    ...
    public void setRefreshing(boolean refreshing) {
        if (mRefreshing != refreshing) {
            ...
            if (mRefreshing) {
                mProgressBar.start();
                swipeHeaderView.setRefreshing();
            } else {
                mLastDirection = Mode.DISABLED;
                swipeHeaderView.setRefreshComplete();
                mProgressBar.stop();
            }
        }
    }
        ...
        if (mTarget == null) {
            if (getChildCount() == 0) {
                throw new IllegalStateException(
                        "SwipeRefreshLayout can host at least one direct child");
            }
            mTarget = getChildAt(getChildCount() - 1);
            swipeHeaderView = (SwipeHeaderView) getChildAt(0);
